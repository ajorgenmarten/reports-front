...
export const All = () => {
    ...
    const columns: GridColDef[] = [
        { field: "author", headerName: "Autor", width: 240, valueGetter: params => params.row.author?.name ?? '👻 Cuenta eliminada ...' },
        ...
        { field: "createdAt", headerName: "Fecha", minWidth: 150, valueGetter: params => new Date(params.row.createdAt).toLocaleDateString('es-ES') },
        { field: "solution", headerName: "Solución", width: 200, sortable: false, valueGetter: params => params.row.solution ? params.row.solution : '-' },
        { field: "updatedAt", headerName: "Fecha solución", minWidth: 150, valueGetter: params => params.row.status ? new Date(params.row.createdAt).toLocaleDateString('es-ES') : '-' },
        { field: "actions", headerName: "", minWidth: 195, sortable: false, filterable: false, renderCell: params => <TableActions row={params.row} render={getdata}/>},
    ]

    const getdata = async(page: number = 0) => {
        const response = await fetcher(`/reports/all?page=${page}`)
